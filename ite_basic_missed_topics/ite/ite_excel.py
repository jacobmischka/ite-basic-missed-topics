...
def write_xlsx_legend(worksheet):
    ...
    worksheet[DEFICIENT_DIFFERENCE_MISSED_COL].offset(
        column=-1
    ).value = "% Missed greater than"
    worksheet[DEFICIENT_DIFFERENCE_MISSED_COL].value = DEFICIENT_DIFFERENCE_MISSED

    worksheet[DEFICIENT_MISSED_COL].offset(column=-3).value = "OR"
    worksheet[DEFICIENT_MISSED_COL].offset(column=-1).value = "% Missed greater than"
    worksheet[DEFICIENT_MISSED_COL].value = DEFICIENT_MISSED
...
def dump_section_xlsx(sections, outpath):
    ...
    ws.merge_cells(
        start_row=row,
        end_row=row,
        start_column=IteItem.CBY_TOTAL_COL,
        end_column=IteItem.CA3_COL,
    )
    ws.cell(row=row, column=IteItem.CBY_DIFF_COL, value="Difference from National Mean")
    ws.merge_cells(
        start_row=row,
        end_row=row,
        start_column=IteItem.CBY_DIFF_COL,
        end_column=IteItem.CA3_DIFF_COL,
    )
    ws.cell(row=row, column=IteItem.CBY_MISSED_COL, value="% Missed")
    ws.merge_cells(
        start_row=row,
        end_row=row,
        start_column=IteItem.CBY_MISSED_COL,
        end_column=IteItem.CA3_MISSED_COL,
    )
    ws.cell(row=row, column=IteItem.CBY_DEFICIENT_COL, value="Deficient Area")
    ws.merge_cells(
        start_row=row,
        end_row=row,
        start_column=IteItem.CBY_DEFICIENT_COL,
        end_column=IteItem.CA3_DEFICIENT_COL,
    )
...
def add_styles(worksheet, data_ranges):
    ...
    for range_start, _ in data_ranges:
        for row in worksheet[
            "{}{}:{}{}".format(
                ...
                range_start - 2,
                get_column_letter(IteItem.CA3_DEFICIENT_COL),
                range_start - 1,
            )
        ]:
            for cell in row:
    ...
    worksheet.cell(row=DATA_START_ROW, column=IteItem.CBY_DEFICIENT_COL).style = heading
    ...
    data_ranges.append([(data_ranges[-1][1] + 3), (data_ranges[-1][1] + 5)])
...
def add_conditional_formatting(worksheet, data_ranges):
    diff_range = get_ranges(
        IteItem.CBY_DIFF_COL, data_ranges, end_col=IteItem.CA3_DIFF_COL, separator=" "
    )
    missed_range = get_ranges(
        IteItem.CBY_MISSED_COL,
        data_ranges,
        end_col=IteItem.CA3_MISSED_COL,
        separator=" ",
    )
